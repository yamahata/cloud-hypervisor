...
mod unit_tests {
    ...
    fn smbios_type0_is_virtual_machine_bit_set() {
        ...
        // Type 0 (BIOS Information) offset 0x13 is "BIOS Characteristics
        // Extension Byte 2"; bit 4 (0x10, IS_VIRTUAL_MACHINE) tells guest
        // drivers (e.g. NVIDIA's aarch64 driver) that they are running under
        // a hypervisor. Read the raw byte off the wire, not the Rust struct
        // field, so this test pins the emitted format rather than merely the
        // struct literal in `setup_smbios`.
