...
// ACPI fixes this SRAT structure at 40 bytes.
...
fn create_acpi_tables_internal(
    ...
    let madt_addr = next_table_address(facp_addr, facp.len() as u64)?;
    ...
    #[cfg(target_arch = "aarch64")]
    {
        ...
        let gtdt_addr = next_table_address(prev_tbl_addr, prev_tbl_len)?;
    ...
    let mcfg_addr = next_table_address(prev_tbl_addr, prev_tbl_len)?;
    ...
    #[cfg(target_arch = "aarch64")]
    {
        ...
        let spcr_addr = next_table_address(prev_tbl_addr, prev_tbl_len)?;
        ...
        let dbg2_addr = next_table_address(prev_tbl_addr, prev_tbl_len)?;
    ...
    if tpm_enabled {
        ...
        let tpm2_addr = next_table_address(prev_tbl_addr, prev_tbl_len)?;
    ...
    if !numa_nodes.is_empty() {
        ...
        let srat_addr = next_table_address(prev_tbl_addr, prev_tbl_len)?;
        tables_bytes.extend_from_slice(srat.as_slice());
        xsdt_table_pointers.push(srat_addr.0);

        // SLIT
        let slit = create_slit_table(numa_nodes);
        let slit_addr = next_table_address(srat_addr, srat.len() as u64)?;
    ...
    let xsdt_addr = next_table_address(prev_tbl_addr, prev_tbl_len)?;
...
pub fn create_acpi_tables(
    ...
    let dsdt_addr = next_table_address(rsdp_addr, Rsdp::len() as u64)?;
...
mod tests {
    ...
    #[test]
    fn test_next_table_address() {
        assert_eq!(
            next_table_address(GuestAddress(0x1000), 0x20).unwrap(),
            GuestAddress(0x1020)
        );
        assert!(matches!(
